...
using BatchedBlockwiseKernelMap = std::unordered_map<int, BatchedBlockwiseKernel>;
...
torch::Tensor batched_gemm_fp8_blockwise_tune(torch::Tensor& XQ,
                                              ...
                                              int kernelId,
                                              int /*splitK*/) {
    static const BatchedBlockwiseKernelMap lookup{GENERATE_LOOKUP_TABLE(DDataType, EDataType)};
...
torch::Tensor batched_gemm_fp8_blockwise_tune(torch::Tensor& XQ,
    ...
    if (Y.dtype() == at::ScalarType::Half) {
        return batched_gemm_fp8_blockwise_tune<FP32, FP16>(XQ, WQ, x_scale, w_scale, Y, kernelId, splitK);
    } else if (Y.dtype() == at::ScalarType::BFloat16) {
        return batched_gemm_fp8_blockwise_tune<FP32, BF16>(XQ, WQ, x_scale, w_scale, Y, kernelId, splitK);
    }
