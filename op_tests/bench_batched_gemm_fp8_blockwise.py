...
def _bench_shapes(name: str, shapes: List[tuple], iters: int) -> None:
    ...
    for B, M, N, K in shapes:
        ...
        triton_us = _bench(
            lambda: aiter.batched_gemm_fp8_blockwise(A, W, A_s, W_s, out=out, backend="triton"),
            iters=iters,
        )
        try:
            ck_us = _bench(
    ...
    _print_table(
        rows,
        headers=["B", "M", "N", "K", "torch us", "triton us", "ck us", "tri/torch", "ck/torch", "tri TFLOPs"],
    )
