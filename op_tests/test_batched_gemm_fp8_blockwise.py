...
def test_einsum_form_bit_exact_with_bmk_form():
    """
    Same Triton kernel, different access stride only -- expect bit-exact match
    ...
    out_einsum = aiter.batched_gemm_fp8_blockwise_einsum(
        "tgd,grd->tgr", A_tgd, As_tgd, W_grd, Ws_grd, backend="triton",
    )
...
def test_einsum_form_no_data_movement():
    ...
    aiter.batched_gemm_fp8_blockwise_einsum("tgd,grd->tgr", A_tgd, As_tgd, W_grd, Ws_grd, backend="triton")
