...
def test_partial_rotary(
    ...
    q_diff = (
        (q_ref.reshape(num_tokens, -1) - q_out.reshape(num_tokens, -1))
        .abs()
        .max()
        .item()
    )
    k_diff = (
        (k_ref.reshape(num_tokens, -1) - k_out.reshape(num_tokens, -1))
        .abs()
        .max()
        .item()
    )
    v_diff = (
        (v_ref.reshape(num_tokens, -1) - v_out.reshape(num_tokens, -1))
        .abs()
        .max()
        .item()
    )
