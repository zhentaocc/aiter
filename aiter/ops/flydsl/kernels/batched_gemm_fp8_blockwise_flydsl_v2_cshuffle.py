...
"""
v2 + CShuffle epilogue: same K-loop as v2, output store goes through LDS.

Goal: convert v2's direct buffer_store_b16 (uncoalesced, 2-byte stores
strided by N) into LDS-staged 16-byte coalesced stores, mirroring CK's
CShuffle epilogue.

Strategy (process 2 rounds of 64 rows each, reusing the dead 16 KB A LDS
buffer as bf16 staging):

  After K-loop done (LDS A buffer is dead from here on):
    for round in (0, 1):
        # Round handles m_sub indices [round*4, round*4+4)
        for ms_local in 0..3:
            for n_sub in 0..1:
                for i in 0..3:
                    bf16 = trunc(accs[m_sub][n_sub][i])
                    cshuffle_lds[ms_local*16 + (lane//16)*4 + i,
                                 wave_n_offset + n_sub*16 + lane%16] = bf16
        gpu.barrier()
        # Cooperative HBM write: 256 thread × 8 bf16 = 16 rows × 128 cols / pass
        # 64 rows / 16 = 4 passes
        for pass_i in 0..3:
            row = pass_i*16 + tid//16; col_start = (tid%16)*8
            data = cshuffle_lds.vec_load((row, col_start), 8)
            Out[..., round*64+row, col_start..col_start+8] = data
        gpu.barrier()  # before next round overwrites LDS

LDS budget: reuses smem_a 16 KB region (viewed as bf16 64x128) → no new
LDS allocation, no occupancy hit vs v2.

Expected gain: 5-15% on prefill from store coalescing (was 64 × 2-byte
stores per lane = 128 B serialized at LSU; now ~4 × 16-byte stores
per round × 2 rounds = same bytes but in 16x bigger transactions).

See OPTIMIZATION_JOURNEY.md Iter 9 for v3 (which tested the K-loop
restructuring and showed it didn't help without sched_group_barrier).
This file isolates the *epilogue* part of CK's lead.
"""
...
def compile_bgfp8bw_v2_cshuffle_kernel(B: int, M: int, N: int, K: int,
                                load_mode: str = "coop"):
    ...
    def kernel(
        ...
        # ====================================================================
        # CShuffle epilogue: VGPR -> LDS -> HBM with coalesced 16-byte stores.
        #
        # Reuse the 16 KB A LDS buffer (dead after K-loop) as a bf16 staging
        # area: view it as shape (64, 128) bf16 = 16 KB.
        #
        # Process 128-row output in 2 rounds of 64 rows each.
        # Per round:
        #   1. Each wave writes its 64 x 32 region to LDS at coalesced
        #      (row, col) positions (4 m_sub * 2 n_sub * 4 i = 32 scalar
        #      bf16 stores per lane).
        #   2. Barrier — wait all waves' LDS writes done.
        #   3. Cooperative HBM write: 256 thread x 8 bf16 = 16 rows x 128 cols
        #      per pass. 64 rows / 16 = 4 passes per round.
        #      Per pass: 1 ds_read_b128 (16 B) + 1 buffer_store_b128 (16 B).
        #   4. Barrier (skip on last round) before next round overwrites LDS.
        # ====================================================================
        # View existing 16 KB A LDS region as 64x128 bf16 (overlapping)
        smem_cshuffle_ptr = SmemPtr(allocator.get_base(), smem_a_offset,
                                      bf16_t, shape=(64 * 128,))
        cshuffle_lds = STensor(smem_cshuffle_ptr, dtype=bf16_t,
                               shape=(64, _BLOCK_N))

        for round_idx in range_constexpr(2):
            m_sub_offset = round_idx * 4   # 0 or 4

            # Step 1: per-lane scalar bf16 stores into LDS at coalesced (m,n) layout
            for ms_local in range_constexpr(4):
                m_sub_global = m_sub_offset + ms_local
                for n_sub in range_constexpr(_N_SUB):
                    for i in range_constexpr(4):
                        lds_row = (fx.Index(ms_local * 16)
                                   + (lane // fx.Index(16)) * fx.Index(4)
                                   + fx.Index(i))
                        lds_col = (wave_n_offset
                                   + fx.Index(n_sub * 16)
                                   + (lane % fx.Index(16)))
                        bf16_val = _truncf_f32_to_bf16(accs[m_sub_global][n_sub][i])
                        cshuffle_lds[lds_row, lds_col] = bf16_val

            # Step 2: barrier — all waves' LDS writes visible
            gpu.barrier()

            # Step 3: cooperative HBM write, 4 passes per round.
            # 256 threads arranged as 16 (M) x 16 (N) cluster, each thread
            # owns 8 contiguous N positions (16 bytes).
            for pass_i in range_constexpr(4):
                thread_row_in_pass = tid // fx.Index(16)        # 0..15
                col_byte_thread = (tid % fx.Index(16)) * fx.Index(8)  # 0,8,...,120
                lds_row = fx.Index(pass_i * 16) + thread_row_in_pass
                m_global = (pid_m * fx.Index(_BLOCK_M)
                            + fx.Index(round_idx * 64)
                            + fx.Index(pass_i * 16) + thread_row_in_pass)
                n_global = pid_n * fx.Index(_BLOCK_N) + col_byte_thread

                # Read 8 bf16 (16 B) from LDS
                data = cshuffle_lds.vec_load((lds_row, col_byte_thread), 8)
                # Write 16 B to HBM
                Out_.vec_store((pid_b, m_global, n_global), data, vec_size=8)

            # Step 4: barrier (skip on last round)
            if round_idx + 1 < 2:
                gpu.barrier()
...
def flydsl_batched_gemm_fp8_blockwise_v2_cshuffle(
    A: torch.Tensor,
    W: torch.Tensor,
    ...
    """v2 + CShuffle wrapper. Same constraints as v2:
    ...
    launcher = compile_bgfp8bw_v2_cshuffle_kernel(B, M, N, K, load_mode=load_mode)
