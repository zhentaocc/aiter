...
Backend selection (this branch ships the flydsl decode path only; the
CK prefill kernel lives on the ``wo_a_fp8_blockwise_ck`` branch):

  * ``"auto"``   -> flydsl single-wave kernel for all shapes that
                    satisfy its constraints (M%16, N%128, K%128); else
                    torch reference. Empirically validated on DSv4 wo_a:
                    flydsl decode wins 5-25x over CK at T<=64.
  * ``"flydsl"`` -> single-wave flydsl kernel (decode-optimized).
...
Backend = Literal["auto", "flydsl", "torch"]

# Once the flydsl loader fails, remember it so we don't re-pay the import
# cost (and the warning spam) on every call.
_FLYDSL_BROKEN: bool = False
...
def batched_gemm_fp8_blockwise(
    ...
    *,
    ...
    Args:
        ...
        backend:  ``"auto" | "flydsl" | "torch"``.  Default ``"auto"``.
    ...
    Notes:
        ...
        * For the equivalent operation under DeepSeek V4 wo_a's
          ...
          ``[T, G, R]`` after.
    """
    global _FLYDSL_BROKEN
    chosen = backend
    if chosen == "auto":
        # Flydsl is the only optimized backend on this branch. Constraints
        # (M%16, N%128, K%128) are checked downstream by the kernel itself.
        chosen = "flydsl"

    if chosen == "flydsl":
        if not _FLYDSL_BROKEN:
            try:
                from aiter.ops.flydsl.kernels.batched_gemm_fp8_blockwise_flydsl import (
                    flydsl_batched_gemm_fp8_blockwise,
                )
                return flydsl_batched_gemm_fp8_blockwise(A, W, A_scale, W_scale, out=out)
            except (ImportError, RuntimeError, AssertionError) as e:
                _log.warning(
                    "flydsl batched_gemm_fp8_blockwise failed (%s); "
                    "falling back to torch (silent for subsequent calls).", e,
                )
                _FLYDSL_BROKEN = True
